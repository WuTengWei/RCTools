...
@implementation FZSwitch {
    float thumbOnPosition;
    float thumbOffPosition;
    float bounceOffset;
    FZSwitchStyle thumbStyle;
...
- (id)init
{
    self = [self initWithSize:FZSwitchSizeNormal
                        style:FZSwitchStyleDefault
                        state:FZSwitchStateOn];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
       self =  [self initWithSize:FZSwitchSizeBig
                     style:FZSwitchStyleDefault
                     state:FZSwitchStateOn];
    }
    return self;
...
- (id)initWithSize:(FZSwitchSize)size state:(FZSwitchState)state
{
    ...
    self.thumbOnTintColor  = [UIColor colorWithRed:255./255. green:255./255. blue:255./255. alpha:1.0];
    self.thumbOffTintColor = [UIColor colorWithRed:255./255. green:255./255. blue:255./255. alpha:1.0];
    self.trackOnTintColor = [UIColor colorWithRed:65./255. green:69./255. blue:89./255. alpha:1.0];
    self.trackOffTintColor = [UIColor colorWithRed:190./255. green:195./255. blue:199./255. alpha:1.0];
...
- (void)switchAreaTapped:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([self.delegate respondsToSelector:@selector(switchStateChanged:)]) {
        if (self.isOn == YES) {
            [self.delegate switchStateChanged:FZSwitchStateOff];
        }
        else{
            [self.delegate switchStateChanged:FZSwitchStateOn];
        }
    }
